...
const OrderBadge: FC<OrderBadgeProps> = ({ status }) => {
    ...
    return (
        <Badge
            ...
            intent={
                status === OrderStatus.RECEIVED ? "greenBold" : status === OrderStatus.COMPLETED ? "green" : (status === OrderStatus.REJECTED || status === OrderStatus.DECLINED) ? "redBold" : status === OrderStatus.PROCESSING ? "blue" : "secondaryBold"
            }
        >
            {
