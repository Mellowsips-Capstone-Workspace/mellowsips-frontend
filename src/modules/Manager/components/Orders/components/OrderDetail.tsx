...
const OrderDetail: FC<OrderDetailProps> = ({ order }) => {
    ...
    return (
        <div className='min-w-full space-x-2 pl-10 text-left'>
            <span
                className="text-slate-500 hover:text-main-primary cursor-pointer"
                onClick={on}
            >
...

            {
                [OrderStatus.RECEIVED as string, OrderStatus.COMPLETED as string].includes(order.status) ? (
                    ...
                    >
                        <ReaderIcon className='inline-block h-5 w-5' />
                    </span>
                ) : null
            }
